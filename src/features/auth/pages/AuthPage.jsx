...
function AuthPage({ mode }) {
  ...
  const config = useMemo(
    () => ({
      "sign-in": {
        title: "Sign in",
        headline: "Welcome back to your premium storefront account.",
        submit: "Continue",
        helper: <Link className="hint-link" to="/forgot-password">Forgot password?</Link>,
      },
      "sign-up": {
        title: "Create account",
        headline: "Create your Optizenqor customer profile.",
        submit: "Create account",
        helper: <Link className="hint-link" to="/sign-in">Already have an account?</Link>,
      },
      "forgot-password": {
        title: "Forgot password",
        headline: "We will send a secure recovery code to your email.",
        submit: "Send code",
        helper: <Link className="hint-link" to="/sign-in">Back to sign in</Link>,
      },
      "verify-code": {
        title: "Verify code",
        headline: "Enter the verification code we sent you.",
        submit: "Verify code",
        helper: <Link className="hint-link" to="/reset-password">Continue to reset</Link>,
      },
      "reset-password": {
        title: "Reset password",
        headline: "Create a new password for your account.",
        submit: "Reset password",
        helper: <Link className="hint-link" to="/sign-in">Return to sign in</Link>,
      },
    }),
    [],
  );
  ...
  async function submit(event) {
    ...
    if (["sign-in", "sign-up", "forgot-password", "verify-code", "reset-password"].includes(mode) && !isEmail(form.email)) {
      nextErrors.email = "Enter a valid email address.";
    }
    if (["sign-in", "sign-up", "reset-password"].includes(mode) && !validateMinLength(form.password, 8)) {
      nextErrors.password = "Use at least 8 characters.";
    }
    if (["verify-code", "reset-password"].includes(mode) && !validateMinLength(form.code, 6)) {
      nextErrors.code = "Enter the 6-digit code you received.";
    }
    if (mode === "reset-password" && form.confirmPassword !== form.password) {
    ...
    try {
      ...
      if (mode === "forgot-password") {
        ...
        window.setTimeout(() => window.location.assign("/verify-code"), 700);
        return;
      }
      ...
      if (mode === "verify-code") {
        ...
        window.setTimeout(() => window.location.assign("/reset-password"), 500);
        return;
      }
      ...
      if (mode === "reset-password") {
        ...
        window.setTimeout(() => window.location.assign("/sign-in"), 700);
      }
    } catch (error) {
      setSubmitted(false);
